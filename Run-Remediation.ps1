...
IMPORTANT: Create the file at the exact path: docs/agentKT/$targetFile
Analyze the actual source code in the repository to populate each section with accurate content.
Follow the same format as docs/agentKT/workflows/01_Protection_Backup.md
"@
            try {
        ...
        elseif ($fileItems[0].Type -eq 'ADD_SECTION' -or $fileItems[0].Type -eq 'ADD_MERMAID') {
            ...
            $prompt = @"
In the file 'docs/agentKT/$targetFile', add these missing sections:
