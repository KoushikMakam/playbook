...
function Build-DomainContracts {
    ...
    if (-not $DocsRoot) {
        ...
        if ($manifestCandidates.Count -gt 0) {
            ...
            $commonRoots = @("docs\agentKT", "docs\knowledge", "docs", "copilot-docs")
    ...
    if (-not $ManifestPath) {
        ...
        if (-not $ManifestPath) {
            $legacyCandidates = @(
                (Join-Path $RepoPath "docs\agentKT\Discovery_Manifest.md"),
                (Join-Path $RepoPath "docs\agentKT\discovery_manifest.md"),
    ...
    foreach ($line in $tableLines) {
        ...
        # Extract relative path from backticks: `workflows/01_Protection_Backup.md`
        $docRelPath = if ($workflowDoc -match '`([^`]+\.md)`') {
            $matches[1]
        } elseif ($workflowDoc -and $workflowDoc -match '\.md') {
            $workflowDoc
        } else {
            # Derive from domain name: "Protection Backup" → "workflows/NN_Protection_Backup.md"
...
Follow the same format as existing workflow docs in docs/agentKT/workflows/.
